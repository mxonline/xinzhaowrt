...
! grep -Eq 'set_if_changed openclash\.config\.(enable_redirect_dns|redirect_dns) 0' "$COORD" || fail 'coordinator still suppresses OpenClash DNS redirect'
grep -Fq 'DNS redirect remains OpenClash-owned' "$COORD" || fail 'coordinator does not document OpenClash DNS ownership'
