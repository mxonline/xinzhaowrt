...
auto='.github/workflows/arthur-update-v3-auto.yml'
...
[[ -f "$auto" ]] || { echo "FAIL: missing $auto" >&2; exit 1; }
...
grep -Fq 'REQUEST_ID: ${{ inputs.request_id }}' "$workflow" || { echo 'FAIL: production workflow must receive the exact durable request id.' >&2; exit 1; }
...
grep -Fq -- '-f request_id="$REQUEST_ID"' "$auto" || { echo 'FAIL: auto trigger must pass the exact durable request id into the build workflow.' >&2; exit 1; }
