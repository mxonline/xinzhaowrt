...
Assert-Contains $wakeup '$env:GITHUB_WORKSPACE = $env:ARTHUR_CONTROL_PLANE_CODE_ROOT' 'release governance, resume-state and event-ledger must bind to the clean current-main control checkout'
Assert-NotContains $wakeup '$env:GITHUB_WORKSPACE = $env:ARTHUR_CONTROL_PLANE_WORKSPACE' 'stale dirty task workspace must never be the release-governance workspace'
Assert-Contains $wakeup 'CONTROL_PLANE_GOVERNANCE_WORKSPACE_BOUND=PASS' 'runtime evidence must prove governance is bound to clean current main'
Assert-Contains $wakeup 'CONTROL_PLANE_TASK_WORKSPACE_PRESERVED=PASS' 'runtime evidence must prove the dirty task workspace remains preserved separately'
...
Write-Host 'ARTHUR_GOVERNANCE_WORKSPACE_CONTRACT=PASS'
