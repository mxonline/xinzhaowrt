...
[[ -f "$init" ]] || { echo "FAIL: missing LuCI migration init service: $init" >&2; exit 1; }
[[ -f "$enable" ]] || { echo "FAIL: missing one-time LuCI migration enable hook: $enable" >&2; exit 1; }

grep -Fq 'chmod +x "$SRC/files/usr/libexec/xinzhao/luci-upgrade-converge.sh"' "$build" || { echo 'FAIL: production assembly must make the convergence helper executable.' >&2; exit 1; }
grep -Fq 'chmod +x "$SRC/files/etc/init.d/xinzhao-luci-migrate"' "$build" || { echo 'FAIL: production assembly must make the migration service executable.' >&2; exit 1; }
grep -Fq 'chmod +x "$SRC/files/etc/uci-defaults/95-xinzhao-luci-migrate-enable-v1"' "$build" || { echo 'FAIL: production assembly must make the migration enable hook executable.' >&2; exit 1; }
...
run_helper() {
  PATH="$tmp/bin:$PATH" TEST_UCI_STATE="$tmp/state" TEST_UCI_LOG="$tmp/uci.log" TEST_LOGGER_LOG="$tmp/logger.log" \
  XINZHAO_BUILD_INFO="$tmp/build-info" XINZHAO_STATE_DIR="$tmp/persist" XINZHAO_UCI="$tmp/bin/uci" XINZHAO_LOGGER="$tmp/bin/logger" \
  sh "$helper"
}
