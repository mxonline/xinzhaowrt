...
grep -Fq '"$SDK_APK" --allow-untrusted index --output packages.adb ./*.apk' "$workflow" || fail 'verified package repository reindex must handle its archived signature scope'
