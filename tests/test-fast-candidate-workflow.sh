...
grep -Fq 'tar -xf "$QUICKSTART_APK" -C quickstart-root' "$workflow" || fail 'QuickStart package extraction must auto-detect its archive format'
! grep -Fq 'tar --zstd -xf "$QUICKSTART_APK"' "$workflow" || fail 'QuickStart package extraction incorrectly forces zstd'
