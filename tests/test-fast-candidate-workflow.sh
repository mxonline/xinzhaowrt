...
grep -Fq '"$SDK_APK" --allow-untrusted mkndx --output packages.adb ./*.apk' "$workflow" || fail 'verified package repository must be reindexed as APKv3'
grep -Fq "printf 'ndx file://%s/packages.adb\\n'" "$workflow" || fail 'ImageBuilder must register the local APKv3 index explicitly'
