...
grep -Fq '"$SDK_APK" extract --destination quickstart-root "$QUICKSTART_APK"' "$workflow" || fail 'QuickStart package extraction must use the matching SDK apk implementation'
