...
Assert-Contains $persistentSupervisorTask 'LogonType Interactive' 'persistent supervisor must retain an interactive fallback for legacy credential-bearing tasks'
Assert-Contains $persistentSupervisorTask 'Principal.LogonType' 'existing Supervisor logon type must be preserved across launcher drift repair'
Assert-Contains $persistentSupervisorTask 'Principal.RunLevel' 'existing Supervisor run level must be preserved across launcher drift repair'
Assert-Contains $persistentSupervisorTask '-LogonType $taskLogonType' 'canonical Supervisor principal must use the preserved logon type'
Assert-Contains $persistentSupervisorTask '-RunLevel $taskRunLevel' 'canonical Supervisor principal must use the preserved run level'
Assert-Contains $persistentSupervisorTask 'New-ScheduledTaskTrigger -AtStartup' 'runner-owned S4U Supervisor must use a startup trigger instead of requiring desktop logon'
Assert-Contains $persistentSupervisorTask "'S4U'" 'persistent Supervisor helper must recognize runner-owned S4U tasks'
...
Write-Host 'ARTHUR_PERSISTENT_SUPERVISOR_S4U_PRESERVATION_CONTRACT=PASS'
