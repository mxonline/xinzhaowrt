...
[[ -s "$manifest" ]] || { echo 'FAIL: accepted QuickStart manifest is missing from the production source.' >&2; exit 1; }
...
grep -Fq 'mature AdGuard CBI manager' "$rootfs_verifier" || { echo 'FAIL: final rootfs verifier must require the accepted mature AdGuard CBI manager.' >&2; exit 1; }
