...
## RELEASE-FIRST automation baseline

The user has explicitly changed the production standard to separate unattended GitHub Release from router flashing and whole-device acceptance. The machine-readable authority is `production/release-mode.json`.

- The only primary workflow is `RELEASE-FIRST AUTOMATION MODE`.
- The only successful production terminal is `PRODUCTION_RELEASED`.
- The default production route is exactly `RELEASE_ONLY`.
- The default `RELEASE_ONLY` production order is: recover current release state → determine minimum change scope → `CHANGE_IMPACT_GATE` → `BASELINE_INHERITANCE_GATE` → `EXPECTED_DIFF_GATE` → choose the fastest reliable build path (`ImageBuilder` / `SDK` / `Full Build`) → Build → artifact/SHA256/manifest/config/plugin/theme checks → `RELEASE_GATE` → GitHub Release → `PRODUCTION_RELEASED`.
- In `RELEASE_ONLY`, `automatic_flash=false`. `PRE_FLASH`, `AUTO_FLASH_SAFETY_GATE`, `FLASH`, `WAIT_DEVICE` and real-device runtime gates remain parseable for historical compatibility but are not actionable production prerequisites.
- `POST_RELEASE_DEVICE_TEST` is independent and non-blocking for GitHub Release. It must not trigger a rebuild, undo a published release, or change the accepted release artifact.
- `production/known-good.json` may advance only after `POST_RELEASE_DEVICE_TEST=PASS` for the exact released firmware/hash. Until then the previous real-device-confirmed Known-Good remains rollback authority.
- `FLASH_AND_VERIFY` is compatibility-only and may be selected only by explicit policy plus explicit device-write authorization. `FIRMWARE_RELEASE` authorization alone never implies router-write permission.
- Do not invent, insert, rename, reorder or promote a new Gate, Agent, Controller, Runtime stage or workflow stage without an explicit product-standard change. Unified resume/event reconciliation is a pre-action state integrity check, not a new production stage.
- Before proposing or executing a next action, `production/operator-intent.json` must authorize that exact scope, `production/resume-state.json` must be `RESUME_SAFE` with `instruction_allowed=true`, `production/firmware-events.jsonl` must validate, and `scripts/arthur-firmware-resume.ps1` (or its exact equivalent) must return `RESUME_GATE_SAFE`; then reconcile the live current stage, the release-mode route, and the proposed action. If they do not match, do not execute the action.
- If repository documents or defaults conflict with current machine-readable/product targets, do not guess. Reconcile the conflicting Source of Truth before starting a new Candidate build; live verified build evidence and explicit current product requirements override stale text.
...
- The recovery target is the Arthur release task itself, not a specific Codex process or thread. After a computer restart, Codex crash, network interruption or controller failure, recover from durable HANDOFF/state plus live GitHub, artifact and release evidence, and continue from the last valid checkpoint.
- Completed or verified stages must not be repeated without evidence that their result is invalid. Reuse an existing GitHub build if still valid and reuse an already verified artifact. A device write is never inferred from this reuse rule.
...
- `CHANGE_IMPACT_GATE` must choose the shortest reliable build path. For same-device/same-target work with compatible source lock and toolchain, prefer ImageBuilder, SDK/package-only rebuilds, source reuse and valid compatibility-keyed caches. Full Build is a fallback when the change impact or verification evidence requires it.
- Do not create probe workflows, Bridge E2E tests, recovery experiments, parallel writers or new orchestration architectures inside an active Arthur production release unless a concrete release blocker cannot be removed by a smaller repair.
...
- `LIVE_PREVIEW` is not a Gate, Candidate, flash stage, formal post-release device test or release stage.
- `LIVE_PREVIEW=PASS` must always coexist with `REAL_DEVICE_VERIFY=NOT_RUN`; preview evidence does not promote Known-Good.
...
- After preview acceptance, freeze source and return to the `RELEASE_ONLY` production sequence. The Candidate may enter `RELEASE_GATE` after firmware/artifact safety evidence passes. Whole-device acceptance runs later as independent `POST_RELEASE_DEVICE_TEST` and gates only Known-Good promotion.
...
`RELEASE_ONLY` never performs automatic router writes. A successful Candidate, Release Gate, or GitHub Release does not authorize `sysupgrade`.

Standard Arthur sysupgrade is available only to an explicitly authorized `FLASH_AND_VERIFY` or independent `POST_RELEASE_DEVICE_TEST` execution. That separate device-write execution must satisfy the project's full device identity, storage-layout, candidate integrity, cloud/local/remote SHA256, plugin/theme/config, current device health, expected LAN and Known-Good rollback safety evidence before any standard sysupgrade.

Never guess sysupgrade arguments. Reuse only the project's previously verified Arthur upgrade parameters when a separately authorized device-write task requires them.

Raw or boot-critical write paths are outside automation. Never automatically perform MTD, U-Boot, bootloader, `dd`, raw eMMC/SPI/NAND writes, partition-table changes, ART/EEPROM/calibration writes, or equivalent raw-storage operations. These require explicit human authorization or remain prohibited according to project safety policy.

If device identity, storage layout, rollback safety, candidate hash or flash state is unknown, stop device-write actions and preserve evidence. This device-write failure does not retroactively invalidate an already published `RELEASE_ONLY` GitHub Release; it prevents Known-Good promotion until `POST_RELEASE_DEVICE_TEST=PASS` for the exact released hash.
