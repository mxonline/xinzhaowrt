...
# A newly authorized RELEASE_ONLY execution must be made durable before the legacy
# Resume Gate sees the previous terminal execution. This path is GitHub-native:
# no user-profile worktree, DPAPI, PAT/JWT bridge, or force push is used.
Push-Location $root
try {
...
& $controlPlanePath -Repository $Repository -WorkflowRunId $WorkflowRunId
exit $LASTEXITCODE
) { throw 'FRESH_BOOTSTRAP_SOURCE_SHA_INVALID' }
