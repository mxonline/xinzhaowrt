...
function Run-Phase([string]$Prefix) {
    ...
    Test-Remote "$Prefix.branding" 'test -s /www/luci-static/xinzhao/logo.png && test -s /www/luci-static/xinzhao/favicon.ico && test -s /www/luci-static/xinzhao/branding.js && { grep -F XinZhaoWrt /www/luci-static/xinzhao/build-info.json 2>/dev/null || grep -F XinZhaoWrt /etc/xinzhao-build-info 2>/dev/null; }' { param($o) $o -match 'XinZhaoWrt' } 'XinZhaoWrt branding/build information must be present.' | Out-Null
    Test-Remote "$Prefix.adguard_manager" 'test -s /etc/config/adguardhome || test -s /etc/config/AdGuardHome; { test -x /etc/init.d/adguardhome || test -x /etc/init.d/AdGuardHome; }; { test -s /www/luci-static/resources/view/adguardhome/config.js || { test -s /usr/lib/lua/luci/controller/AdGuardHome.lua && test -d /usr/lib/lua/luci/model/cbi/AdGuardHome; }; }' { param($o) $true } 'Complete AdGuard Home manager, config and service must be present through the deployed modern or uppercase CBI implementation.' | Out-Null
